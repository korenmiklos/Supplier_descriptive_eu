...
\cortext[cor2]{The authors gratefully acknowledge funding from the European Research Council (ERC Starting Grant agreement number 313164) and XXX. }
...
\section{ Survey}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Setting up the survey}

Doing a survey is difficult for many reasons: devsising the questionnaire, collecting data - doing the survey itself, and managing datasets.

First, formulating questions that can be reasonably answered and yet helps us get closer to research questions takes experimentation. To create a survey that works in more countries, required understanding some legal and accounting principals, too. To support our efforts, for our survey, we could build on earlier experience in international surveys -- we were partners and team leaders in EFIGE (\url{https://bruegel.org/publications/datasets/efige/}), one of the earliest and most successful survey projects on firms. 

Second, we needed a partner to carry out the survey, decising the platform to collect answers, digitize them, and co-ordinate an array of poeple visiting firms. We decided to opt for this rather expensive approach in order to show our seriousness towards companies, hoping to get some facetime from senior management. Given the sensitive nature of some questions, gaining trust was essentual. 

We set up an open call for companies with presence in our target countries, and after a two-round process, GfK emerged as winner. Their Budapest office won the contract co-ordinating data collection in Hungary, Slovakia and Romania using GfK's Slovakian and Romanian affiliates. 

Finally, at CEU, we have a center for managaing microlevel datasets.

%MIKLOS -  2 sentences here

\subsection{computer assisted personal interview}
We considered several options how to collect data. Eventually we opted for a computer assisted personal interview. Surveyors visited firms with a tablet where they could show questions and answer them. 

Surveyors were appointed and trained by GfK, but we also gave a short talk to surveyors, and helped them understand questions. We had numerous calls with project managers from all countries. 

Surveyors visited firms located in a given region of the country. This means there was no way to randomize surveyors across respondents. Fortunately, there was not much room for surveyor subjective interpretation, so this seemed less of a concern. 
...
In this section, let us present some key questions we asked. 

First, some questions were similar to both suppliers and customers as reported in Table~\ref{tab:Q1}.

%%%%%%%%%% TABLE Q1
\begin{table}[H]
\caption{Symmetric buyer/supplier questions}
\label{tab:Q1}
\centerline{
...
Some of our questions were more sensitive asking about the relationship between respondent and their business partners. This where we focused on the nature of collaboration and types of products they trade. A few examples are shown in Table~\ref{tab:Q2} 
...
Many of relationships were part of dealings in a multinational group, and we askes some basic questions about it, with examples in Table~\ref{tab:Q3}
...
Beyond buyers and suppliers, some of the questionnare was devoted to understanding high valued machinery use as shown in Table~\ref{tab:Q4}.
...
Finally, we asked a few questions about the company itself, with some examples presented in Table~\ref{tab:Q5}
...
\begin{table}[h]
    \caption{Firm characteristics and relationship length (years)}
    \label{tab:char_length}    
    \input{tables/Table9a.tex}

    %% FIXME: Table 9a is not about length! 

    {\scriptsize \textit{Notes:} The figure shows the median length of key relationships (each such relationship when respondent is supplier is one observation). SME: $\le 50$ employees, large otherwise.}
\end{table}
